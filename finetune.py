...
def make_dataset(source_name, target_name, max_seconds=12):
    dataset = Dataset.load_from_disk(
        f"datafast/{source_name}.train.trim_{max_seconds}.hf"
    )
    source_dev_dataset = Dataset.load_from_disk(f"datafast/{source_name}.dev.hf")
    ...
    print(f"LOOOOG: Finetuning on {source_name} source domain")

    return dataset, source_dev_dataset, target_dev_dataset, test_dataset
...
def make_training_args(source_name, target_name, args):
    training_args = TrainingArguments(
        ...
        output_dir=f"./xlsr-finetuned-src_{source_name}-tgt_{target_name}",
...
def main():
    ...
    train_dataset, source_dev_dataset, target_dev_dataset, test_dataset = make_dataset(
